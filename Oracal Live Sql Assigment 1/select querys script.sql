REM   Script: select Query Script
REM   select Query Script
...
FROM ORDERS O 
JOIN CUSTOMERS C ON O.CUSTOMERID = C.CUSTOMERID;SELECT O.ORDERID, O.ORDERDATE, O.PRODUCTNAME, O.QUANTITY, O.TOTALAMOUNT, C.CUSTOMERID, C.FIRSTNAME, C.LASTNAME, C.EMAIL, C.PHONENUMBER 
FROM ORDERS O  
...
FROM ORDERS O 
JOIN CUSTOMERS C ON O.CUSTOMERID = C.CUSTOMERID 
WHERE O.ORDERDATE >= TO_DATE('2023-01-01', 'YYYY-MM-DD') 
...
FROM ORDERS O 
JOIN CUSTOMERS C ON O.CUSTOMERID = C.CUSTOMERID 
WHERE O.ORDERDATE >= TRUNC(SYSDATE, 'MM') - INTERVAL '1' MONTH 
